    ...
    Single<String> getGigyaSession(final String username, final String password) {
        if (this.securityData != null && this.securityData.getGigyaApiKey() != null) {
            Single<String> stringSingle = Single.create((SingleOnSubscribe<String>) emitter -> {
                ...
                this.getDataFromGigyaFramework("login", params, emitter, response -> {
                    emitter.onSuccess(response.getJSONObject("sessionInfo")
        ...
        if (this.securityData != null && this.securityData.getGigyaApiKey() != null) {
            if (this.securityData.isGigyaJwtExpired()) {
                Single<String> stringSingle = Single.create((SingleOnSubscribe<String>) emitter -> {
                    ...
                    this.getDataFromGigyaFramework("getJWT", params, emitter, response -> {
                        String token = response.getString("id_token");
                        Log.d(TAG, "new Gigya token = " + token);
                        this.securityData.setGigyaJwt(token);
                        emitter.onSuccess(token);
                    });
                });
                return stringSingle;
            } else {
                Log.d(TAG, "old Gigya token = " + this.securityData.getGigyaJwt());
                return Single.just(this.securityData.getGigyaJwt());
            }
        } else {
            return Single.error(new IllegalStateException("no security data available"));
        }
    ...
    Single<String> getGigyaPersonId() {
        if (this.securityData != null && this.securityData.getGigyaApiKey() != null) {
            Single<String> stringSingle = Single.create((SingleOnSubscribe<String>) emitter -> {
                Map<String, String> params = new HashMap<>();
                params.put("oauth_token", this.securityData.getGigyaSessionToken());
                this.getDataFromGigyaFramework("getAccountInfo", params, emitter, response -> {
                    emitter.onSuccess(response.getJSONObject("data")
                                              .getString("personId"));
                });
            });
            return stringSingle;
